...
***************************************/
        ...
        ***************************************/
                function register($file_id, $var_name, $content){
                        ...
                        for(reset($file_ids); $file_id = trim(current($file_ids)); next($file_ids)){
                                ...
                                if(isset($this->var_names[$file_id]) AND count($this->var_names[$file_id]) > 0){
                                        for($i=0; $i<count($this->var_names[$file_id]); $i++){
                                                ...
                                                $this->files[$file_id] = str_replace($this->start.$temp_var.$this->end, $this->vars[$temp_var], $this->files[$file_id]);
                                        }
                                }
                        }
                }
        ...
        ***************************************/
                function parse_loop($file_id, $array_name, &$array){
                        $loop_code = '';
                        ...
                        if($loop_code != ''){
                                ...
                                for($i=0; $i<count($array); $i++){
                                        $temp_code = $loop_code;
                                        while(list($key,) = each($array[$i])){
                                                $temp_code = str_replace($this->start.$key.$this->end,$array[$i][$key], $temp_code);
                                        }
                                        $new_code .= $temp_code;
                                }
